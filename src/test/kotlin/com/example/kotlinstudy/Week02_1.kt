...
class Week02 {
    ...
    fun test01() {
        ...
        // FIXME
        val result = studentList.asSequence()
            .filter { it.grade <= 'B' }
            .filter { it.age in 12..20 }
            .sortedBy { it.name }
            .groupBy { it.classNo }
    ...
    fun test02() {
        ...
        personMap.flatMap { it.value }
            .forEach {
                if (it.age % 4 == 2)
                    assert(it.toPhoneNumber() == "no-phone")
                else
                    assert(it.toPhoneNumber() == "010-123-456")
            }
    }

    // FIXME : 만약 핸드폰이 없다면 <no-phone>이라고 출력하고 핸드폰이 있다면 번호를 출력하라. return type은 맞춰서 변환해라
    fun Person.toPhoneNumber(): String {
        return if (this.phoneNumber.isNullOrEmpty()) "no-phone" else this.phoneNumber
    }
